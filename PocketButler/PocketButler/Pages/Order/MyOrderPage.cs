...
namespace PocketButler
{
    public class MyOrderPage : BasePage
    {
		...
		class OrderHistoryItemDetail
		{
			...
			public OrderHistoryItemDetail(String restaurantname, String ref_id, String id, String order_date, String total_amount, String order_status, Utils.ItemClickedDelegate tappedEvent){
			...
			for (int i = 1; i <= totalOrderPageCount; i++) {
				...
				if (isResultSuccess) {
					...
					foreach (OrderHistory item in resultList) {
						...
						OrderHistoryItemDetail ItemDetail = new OrderHistoryItemDetail (item.restaurant_name, item.order_ref_id, item.order_id, item.orderdate, item.totalamount, item.orderstatus, null);
		...
		private StackLayout GetOrderHistoryItem(){
			...
			{
				WidthRequest = (IsEditing == true) ? 40 : 0,
				VerticalOptions = LayoutOptions.FillAndExpand,
			};
