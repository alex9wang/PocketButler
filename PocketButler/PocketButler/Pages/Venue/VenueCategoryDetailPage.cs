...
namespace PocketButler
{
	public class VenueCategoryDetailPage : BasePage
	{
		class CategoryItemDetail
		{
			...
			public CategoryItemDetail(String id, String text, String textdetail, String icon, String distance, String isoffline)
			{
				this.Id = id;
				this.Text = text;
				...
				if (String.IsNullOrEmpty(isoffline) || isoffline.Equals("0"))
		...
		List<CategoryItemDetail> CategoryDetailList = new List<CategoryItemDetail>();
		...
		async void RefreshCategoryList()
		{
			...
			if (isResultSuccess) {
				...
				foreach (RestaurantInfo item in RestaurantItems) {
					CategoryDetailList.Add (new CategoryItemDetail (item.id, item.name, item.address, item.image, item.distance, item.is_offline));
				}
