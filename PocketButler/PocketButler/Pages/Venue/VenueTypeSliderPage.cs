...
namespace PocketButler
{
	public class VenueTypeSliderPage : BasePage
	{
		...
		private void BuildUI()
		{
			...
			if (RestaurantData.is_offline == "0") {
				if (RestaurantData.settings.is_pickup_enabled.Equals ("1") || RestaurantData.settings.is_tableservice_enabled.Equals ("1")) {
					...
					MakeCustomNavigationBar (UILayout, MenuImage, null, TitleLogoImage, true);
				}
			} else {
			...
			if (RestaurantData.is_offline.Equals ("0") && (RestaurantData.settings.is_pickup_enabled.Equals ("1") || RestaurantData.settings.is_tableservice_enabled.Equals ("1")))
				ItemSliderControl.is_online = true;
			else
				ItemSliderControl.is_online = false;
		...
		async void SliderListView_Category_SubItem_Clicked(String category_id, String item_id, MenuItem item_data)
		{
			...
			await Navigation.PushAsync (new VenueCustomizePage (item_id, isOnline, "", -1, false, PageShowingEvent));
		}
		...
		void SliderListView_Category_AddItem_Clicked(String category_id, String item_id, MenuItem item_data)
		{
			int curTime = DateTime.Now.Millisecond;
			if (Math.Abs (curTime - dlastTimeCalled) < dEqualLimit)
				return;
			...
			dlastTimeCalled = curTime;
			App._DbManager.AddPaymentItem(Globals.Config.RestaurantId, Globals.Config.RestaurantName, item_id, item_data.item_name, item_data.item_price, new List<MenuExtraItem>(), new List<MenuTypeItem>(), "", 1);
