...
namespace PocketButler
{
	public class CreateAccountPage : BasePage
	{
        ...
        CustomDatePicker UserBirthdayEntry;
        ...
        private void BuildUI()
        {
            ...
            AbsoluteLayout.SetLayoutBounds(UserImage, new Rectangle(0.5, 0.2, 0.5, 0.15));
            MainLayout.Children.Add(UserImage);

			TopLabel = new DILabel
			{
				Text = "Create new account with:",
            ...
            AbsoluteLayout.SetLayoutBounds(UserLastNameEntry, new Rectangle(1, 0.482, 0.98, 0.05));
            MainLayout.Children.Add(UserLastNameEntry);

			if (DateOfBirth == null) {
				DateOfBirth = new DateTime (1990, 6, 15);//DateTime.Now;
			}			

			UserBirthdayEntry = new CustomDatePicker
            {
                TextColor = Color.White,
                BackgroundColor = Color.Transparent,
            ...
            AbsoluteLayout.SetLayoutBounds(ConfirmPasswordEntry, new Rectangle(1, 0.728, 0.98, 0.05));
        ...
        {
			if (String.IsNullOrEmpty (UserFirstNameEntry.Text)) {
				await DisplayAlert ("Warning", "Please enter your first name", "OK");
				return;
			}

			if (String.IsNullOrEmpty (UserLastNameEntry.Text)) {
				await DisplayAlert ("Warning", "Please enter your last name", "OK");
				return;
			}

			if (UserBirthdayEntry.Date == null || UserBirthdayEntry.IsDateSelected == false) {
				await DisplayAlert ("Warning", "Please enter your date of birth", "OK");
				return;
			}
			...
			var response = await LoginServices.RegisterUser (Globals.Config.USER_EMAIL, PasswordEntry.Text, UserFirstNameEntry.Text, UserLastNameEntry.Text, UserBirthdayEntry.Date.ToString("dd/MM/yyyy"), device_token, "Android", profileImgFileName);
