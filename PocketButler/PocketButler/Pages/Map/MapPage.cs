...
namespace PocketButler
{
	public class MapPage : BasePage
	{
		...
		private void BuildUI()
		{
			...
			map = new CustomMapView(MapSpan.FromCenterAndRadius(new Position(dLatitude, dLongitude), Distance.FromMiles(0.5)))
			{
		...
		async void PinSelected()
		{
			if (map.SelectedPin != null && categoryItems != null) {
				try{
					foreach (RestaurantInfo item in categoryItems)
					{
						if (map.SelectedPin.Label.Equals(item.name))
						{
